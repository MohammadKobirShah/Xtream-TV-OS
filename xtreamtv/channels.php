...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!Security::verifyCsrf($_POST['csrf_token'] ?? '')) {
        ...
        header("Location: /xtreamtv/channels.php?playlist_id={$playlistId}");
        exit;
    }
    ...
    if ($action === 'add') {
        ...
        if (empty($name) || empty($url)) {
            ...
            View::flash('error', 'Stream URL blocked by SSRF protection.');
        } else {
            Database::query(
                "INSERT INTO channels (playlist_id, name, stream_url, logo, group_title) VALUES (?, ?, ?, ?, ?)",
...
// Ensure user owns the playlist (or is admin)
if ($playlistId) {
...
$total    = (int)Database::query(
    "SELECT COUNT(*) FROM channels c JOIN playlists p ON p.id = c.playlist_id WHERE {$whereStr}",
    $params
)->fetchColumn();
...
// Playlists for add-channel dropdown
$myPlaylists = Database::query("SELECT id, name FROM playlists WHERE user_id = ?", [$userId])->fetchAll();
...
<div class="flex items-center justify-between mb-6">
  <div>
    ...
    <div class="text-muted" style="font-size:0.82rem;margin-top:2px">
      ...
      <?= $playlistId ? ' in <strong>' . Security::e($pl['name'] ?? '') . '</strong>' : '' ?>
    </div>
  </div>
...
<div class="glass mb-4" style="padding:16px 20px;">
  <form method="GET" action="/xtreamtv/channels.php" style="display:flex;gap:12px;flex-wrap:wrap;align-items:flex-end;">
    ...
    <div style="flex:1;min-width:180px;">
      ...
      <input type="text" name="q" placeholder="Search channels..." value="<?= Security::e($search) ?>">
    </div>
    <?php if (!empty($groups)): ?>
    <div style="min-width:180px;">
      ...
      <select name="group">
        ...
        <option value="<?= Security::e($g) ?>" <?= $group === $g ? 'selected' : '' ?>><?= Security::e($g) ?></option>
        <?php endforeach; ?>
      </select>
    </div>
    <?php endif; ?>
  ...
  <div class="table-wrap">
    <table>
      ...
      <?php else: ?>
        ...
        <?php
          $proxyUrl = APP_URL . '/xtreamtv/proxy.php?id=' . $ch['id'] . '&t=' . urlencode($_SESSION['token'] ?? '');
        ?>
        <tr>
          ...
          <td>
            <div style="display:flex;align-items:center;gap:10px;">
              <?php if ($ch['logo']): ?>
              <img src="<?= Security::e($ch['logo']) ?>" alt="" style="width:32px;height:22px;object-fit:contain;border-radius:4px;background:#111;" onerror="this.style.display='none'">
              <?php else: ?>
              ...
              <?php endif; ?>
              <div style="font-weight:600;font-size:0.88rem"><?= Security::e($ch['name']) ?></div>
            </div>
          </td>
          <td><span style="font-size:0.78rem;color:var(--neon-purple)"><?= Security::e($ch['group_title']) ?></span></td>
          <td style="font-size:0.78rem;color:var(--text-muted)"><?= Security::e($ch['playlist_name']) ?></td>
          <td>
            <?php if ($ch['is_active']): ?>
          ...
          <td>
            <span class="mono text-muted" style="font-size:0.7rem;max-width:140px;display:block;overflow:hidden;text-overflow:ellipsis;white-space:nowrap" title="<?= Security::e($ch['stream_url']) ?>">
              <?= Security::e(substr($ch['stream_url'], 0, 30)) ?>…
            </span>
          </td>
          <td>
            <button class="btn btn-ghost btn-sm copy-btn" onclick="navigator.clipboard.writeText('<?= Security::e($proxyUrl) ?>').then(()=>{this.textContent='✓ Copied!';setTimeout(()=>this.textContent='📋 Copy',1500)})">📋 Copy</button>
          </td>
          <td>
            <div class="flex gap-2">
              <a href="<?= Security::e($proxyUrl) ?>" target="_blank" class="btn btn-ghost btn-sm" title="Test stream">▶</a>
              ...
              <form method="POST" style="display:inline">
                ...
                <input type="hidden" name="channel_id" value="<?= $ch['id'] ?>">
                <button type="submit" class="btn btn-danger btn-sm" onclick="return confirm('Delete this channel?')">🗑</button>
              </form>
              <?php endif; ?>
            </div>
          </td>
      ...
      <div class="modal-body">
        ...
        <div class="form-group">
          ...
          <select name="playlist_id" required>
            ...
            <option value="<?= $p['id'] ?>" <?= $playlistId === (int)$p['id'] ? 'selected' : '' ?>><?= Security::e($p['name']) ?></option>
            <?php endforeach; ?>
          </select>
        </div>
        <div class="form-group">
